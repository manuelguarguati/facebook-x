    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 py-6">
      <header>
        <h1 className="text-3xl font-black text-white flex items-center gap-3">
          <ImageIcon className="text-purple-500 h-8 w-8" /> Estudio de Imágenes IA
        </h1>
        <p className="text-neutral-400 mt-1">Genera y edita imágenes con IA para tus publicaciones.</p>
      </header>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {mode === 'generate' ? (
            <>
              <Card className="bg-neutral-950 border-white/5">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm text-white">Describe tu imagen</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div>
                    <label className="text-xs text-neutral-500 uppercase font-bold block mb-2">Proporción</label>
                    <div className="grid grid-cols-2 gap-2">
                      {ASPECT_RATIOS.map(r => (
                        <button
                          key={r.value}
            ...
            <>
              <Card className="bg-neutral-950 border-white/5">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm text-white">Sube una imagen</CardTitle>
                </CardHeader>
                <CardContent>
                  {uploadedImage ? (
              ...
              <Card className="bg-neutral-950 border-white/5">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm text-white">Instrucción de edición</CardTitle>
                </CardHeader>
                <CardContent>
                  <textarea
                    value={editPrompt}
                    onChange={(e) => setEditPrompt(e.target.value)}
                    placeholder="Ej: Cambia el fondo a un atardecer, agrega el logo en la esquina..."
            ...
            <CardContent className="flex-1 flex flex-col items-center justify-center p-6 gap-6">
              {loading ? (
                ...
                <div className="w-full space-y-4">
                  ...
                  {/* Post caption */}
                  <div>
                    <label className="text-xs text-neutral-500 uppercase font-bold block mb-2">Texto del post</label>
                    <textarea
                      value={postContent}
                      onChange={(e) => setPostContent(e.target.value)}
                      placeholder="Escribe el texto que acompañará la imagen..."
                  ...
                  {/* Page selector */}
                  <div>
                    <label className="text-xs text-neutral-500 uppercase font-bold block mb-2">Publicar en</label>
                    <select
