...
export function NewsList({ news }: { news: NewsItem[] }) {
  ...
  const handleCreatePost = (item: NewsItem) => {
    // Send to AI Studio with context
    const params = new URLSearchParams();
    params.set('details', item.title);
    params.set('source', item.source);
    params.set('type', 'Noticia viral del día');
    router.push(`/dashboard/studio/ai?${params.toString()}`);
  };
          ...
          <div className="p-5 flex-1">
           ...
           <div className="p-4 border-t border-neutral-100 dark:border-white/5 bg-neutral-50 dark:bg-neutral-900/50">
              <button 
                ...
                className="w-full py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95"
              >
