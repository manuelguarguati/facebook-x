...
export function ScheduledList({ posts }: { posts: any[] }) {
  ...
  return (
    <div className="space-y-4">
      {posts.map((post) => (
        <div key={post.id} className="p-4 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-white/5 shadow-sm">
          <p className="text-sm text-neutral-800 dark:text-neutral-300 whitespace-pre-wrap">{post.content}</p>
          <div className="flex justify-between items-center mt-4 pt-4 border-t border-neutral-100">
            <div className="flex items-center space-x-2 text-xs">
              <span className={`px-2 py-1 rounded-full font-medium ${post.status === 'scheduled' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>
                {post.status === 'scheduled' ? t('scheduler.status_scheduled') : t('scheduler.status_posted')}
              </span>
              <span className="text-neutral-500 dark:text-neutral-400">Page: {post.platform_id}</span>
            </div>
